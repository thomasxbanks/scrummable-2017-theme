...
gulp.task('default', function(callback) {
	...
	envProd = false
	runSequence('clean:wp-content', 'sass', 'html', 'php', 'js', 'img', 'data', 'include', 'txt', callback)
})

// Watching for changes
gulp.task('watch', function(callback) {
	runSequence('clean:wp-content', 'default', function() {
		browserSync.init({
			server: "wp-content"
		})
...
gulp.task('production', function(callback) {
	...
	envProd = true
	runSequence('clean:wp-content', 'sass', 'html', 'php', 'js', 'img', 'data', 'include', 'txt', () => {
		console.log('production build finished')
	})
})

// Delete the distribution folder
gulp.task('clean:wp-content', function() {
	return gulp.src('./wp-content', {
			read: false
		})
		.pipe(clean())
})
